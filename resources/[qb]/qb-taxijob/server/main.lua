    ...
    if Player.PlayerData.job.name == Config.jobRequired then
        if NearTaxi(src) then
            ...
            if Config.Management then
                exports['qb-management']:GetAccount('taxi', payment, 'Customer payment')
            else
            ...
            if chance < 26 then
                ...
                TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.Rewards], 'add')
            end
        else
