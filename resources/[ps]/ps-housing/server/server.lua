...
function RegisterProperty(propertyData, preventEnter, source)
    ...
    if propertyData.apartment and not preventEnter then
        ...
        if GetResourceState('ps-inventory') == 'started' then
            ...
            print("Error: ps-inventory is not started")
        end
    end
...
lib.callback.register("ps-housing:cb:inventoryHasItems", function(source, name, isOx)
    ...
    local query = lib.checkDependency('ps-inventory', '2.0.0') and 'SELECT items FROM inventories WHERE identifier = ?' or 'SELECT items FROM stashitems WHERE stash = ?'
...
RegisterNetEvent("ps-housing:server:createApartmentStash", function(citizenId, propertyId)
    ...
    local query = lib.checkDependency('ps-inventory', '2.0.0') and 'SELECT items, identifier FROM inventories WHERE identifier' or 'SELECT items, stash FROM stashitems WHERE stash'
    local success, result = pcall(function()
